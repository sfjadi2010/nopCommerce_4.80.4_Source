...
function bindBootstrapTabSelectEvent(tabsId, inputId) {
  $("#" + tabsId + ' > div ul li a[data-toggle="pill"]').on(
    "shown.bs.tab",
    function (e) {
...
function initNestedSetting(
  parentSettingName,
  parentSettingId,
  nestedSettingId
) {
  var parentFormGroup = $('input[name="' + parentSettingName + '"]').closest(
    ".form-group"
  );
  ...
  if (
    $("#" + nestedSettingId + " .form-group").length ==
  ...
  $('input[name="' + parentSettingName + '"]').click(
    {
      ...
      parentFormGroupId: parentFormGroupId,
    },
    parentSettingClick
  );
...
function WrapAndSaveBlockData(card, collapsed) {
  ...
  saveUserPreferences(
    rootAppPath + "admin/preferences/savepreference",
    hideAttribute,
    collapsed
  );
}
...
function prepareTableCheckboxes(masterCheckbox, childCheckbox) {
  ...
  $(childCheckbox).change(function () {
    $(masterCheckbox).prop(
      "checked",
      $(childCheckbox + ":not(:checked)").length === 0 ? true : false
    );
  });
